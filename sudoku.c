#include <stdio.h>
#include <stdlib.h>
#include "list.h"


typedef struct{
   int sudo[9][9];
}Node;

Node* createNode(){
  Node* n=(Node*) malloc(sizeof(Node));
  return n;
}

Node* copy(Node* n){
    Node* new=(Node*) malloc(sizeof(Node));
    *new = *n;
    return new;
}

Node* read_file (char* file_name){
  Node* n = createNode();
  FILE* file = fopen (file_name, "r");
  int i,j;
  for(i=0;i<9;i++){
       for(j=0;j<9;j++){
          if(!fscanf (file, "%d", &n->sudo[i][j]))
            printf("failed to read data!");
       }
  }

  fclose (file);
  return n;
}

void print_node(Node* n){
    int i,j;
    for(i=0;i<9;i++){
       for(j=0;j<9;j++)
          printf("%d ", n->sudo[i][j]);
       printf("\n");
    }
    printf("\n");
}

int is_valid(Node* n){
  int i, j, k, l;
  // Validar filas
  for (i = 0; i < 9; i++) {
      int visto[10] = {0};
      for (j = 0; j < 9; j++) {
          int num = n->sudo[i][j];
          if (num != 0) {
              if (visto[num]) return 0; 
              visto[num] = 1; 
          }
      }
  }
  // Validar columnas
  for (j = 0; j < 9; j++) {
      int visto[10] = {0};
      for (i = 0; i < 9; i++) {
          int num = n->sudo[i][j];
          if (num != 0) {
              if (visto[num]) return 0;
              visto[num] = 1;
          }
      }
  }
  // Validar submatrices de 3x3
  for (i = 0; i < 9; i += 3) {
      for (j = 0; j < 9; j += 3) {
          int visto[10] = {0};
          for (k = 0; k < 3; k++) {
              for (l = 0; l < 3; l++) {
                  int num = n->sudo[i + k][j + l];
                  if (num != 0) {
                      if (visto[num]) return 0;
                      visto[num] = 1;
                  }
              }
          }
      }
  }
  return 1;
}


List* get_adj_nodes(Node* n){
  List* list = createList();
  int i, j;
  for (i = 0; i < 9; i++) {
      for (j = 0; j < 9; j++) {
          if (n->sudo[i][j] == 0) {
              for (int val = 1; val <= 9; val++) {
                  Node* new_node = copy(n);
                  new_node->sudo[i][j] = val;
                  if (is_valid(new_node)) {
                      pushBack(list, new_node);
                  } else {
                      free(new_node);
                  }
              }
              return list;
          }
      }
  }
  return list;
}


int is_final(Node* n){
    int i, j;
    for (i = 0; i < 9; i++) {
        for (j = 0; j < 9; j++) {
            if (n->sudo[i][j] == 0) {
                return 0; 
            }
        }
    }
    return 1;
}

Node* DFS(Node* initial, int* cont){
    Stack* S = createStack(); // Crear la pila
    push(S, initial); // Insertar el nodo inicial en la pila
    *cont = 0; // Inicializar el contador de iteraciones

    while (!is_empty(S)) { // Mientras la pila no esté vacía
        Node* current = top(S); // Obtener el nodo en la cima de la pila
        pop(S); // Eliminar el nodo de la pila
        (*cont)++; // Incrementar el contador de iteraciones

        if (is_final(current)) { // Verificar si es un estado final
            return current; // Retornar el nodo si es solución
        }

        List* adj_nodes = get_adj_nodes(current); // Obtener nodos adyacentes
        Node* adj;
        for (adj = first(adj_nodes); adj != NULL; adj = next(adj_nodes)) {
            push(S, adj); // Agregar cada nodo adyacente a la pila
        }

        free(current); // Liberar la memoria del nodo actual
    }

    return 1;
}



/*
int main( int argc, char *argv[] ){

  Node* initial= read_file("s12a.txt");;

  int cont=0;
  Node* final = DFS(initial, &cont);
  printf("iterations:%d\n",cont);
  print_node(final);

  return 0;
}*/